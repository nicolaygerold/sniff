...
export interface SniffOptions {
  /** Path to sniff binary (default: auto-detect) */
  binPath?: string;
...
/**
 * Get the path to the sniff binary
 */
export function getBinPath(): string {
  // Check for binary in package
  const packageBin = join(__dirname, "..", "bin", "sniff");
...
export class Sniff extends EventEmitter {
  ...
  async init(directory: string): Promise<void> {
    const binPath = this.options.binPath ?? getBinPath();
    const args = ["--json"];
